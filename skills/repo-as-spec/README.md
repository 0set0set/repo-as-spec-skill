...
`npx skills` auto-detects your harness and installs into the right directory.
For pinned versions and a manual copy/symlink fallback, see
...
│   ├── README.md             # Index: which reference to read, and when
│   ├── reference.md          # Theory + detailed discovery method
│   ├── templates.md          # Copy-ready artifacts + decision tree
│   ├── checklists.md         # Discovery, fresh session test, gap audit, report
│   ├── examples.md           # Worked before/after audit fragments
│   ├── architecture-lens.md  # Style, boundaries, dependency rule, trade-offs (deep mode)
│   ├── history-mining.md     # Read-only Git/PR archaeology (deep mode)
│   └── quality-lenses.md     # Review lenses + fitness-function guidance (deep mode)
...
├── CHANGELOG.md              # Versioned history (semantic-release)
└── LICENSE                   # MIT
```

Reference files split into a **lite** path (always relevant: `reference.md`,
`templates.md`, `checklists.md`, `examples.md`) and a **deep** path for mature or
decision-heavy repos (`architecture-lens.md`, `history-mining.md`,
`quality-lenses.md`). See [references/README.md](references/README.md).
